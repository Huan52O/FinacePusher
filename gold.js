...
const getMarketTrend = () => {
  ...
  // ct=88b4e9cb013af3511f9055c36cb00aef&tag=123&_token=ImUhgMBh51si6WEUrC2tw5YDJOZGBjWKOebvkD4m
  data.append("ct", "88b4e9cb013af3511f9055c36cb00aef");
  data.append("tag", "123");
  data.append("_token", "ImUhgMBh51si6WEUrC2tw5YDJOZGBjWKOebvkD4m");

  return new Promise((resolve, reject) => {
    axios({
      method: "post",
      url: "http://hq.gzhj99.com/api/getSelfData",
      data: data,
      headers: {
    ...
    <div style="padding:20px 15px;">
      ${list
        .map((item, i) => {
          ...
          <div style="display:flex; align-items:center; margin-bottom:10px; border-bottom:1px solid #00f7ff33; padding-bottom:8px;">
            <img src="${url}" style="width:32px; height:32px; margin-right:12px;">
            <span style="color:#fff; font-size:18px; font-weight:700;">${item.name}</span>
          </div>
          <div style="display:grid; grid-template-columns:repeat(2, 1fr); gap:10px;">
            ${item.children.map(child => {
              return `<div>
                        <span style="color:${titleColor}; font-size:16px; font-weight:600;">${child.name}</span>
                        <div style="color:${textColor}; font-size:22px; font-weight:700;">${child.price}</div>
                      </div>`
              }).join('')}
          </div>
        </div>`;
...
const main = async () => {
  ...
  const resForward = JSON.parse(
    forwards.substring(forwards.indexOf("(") + 1, forwards.lastIndexOf(")"))
  );
